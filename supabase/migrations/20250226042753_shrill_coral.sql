...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'profiles_user_id_role_unique_new'
  ) THEN
    ALTER TABLE profiles DROP CONSTRAINT profiles_user_id_role_unique_new;
  END IF;
...
CREATE UNIQUE INDEX idx_profiles_user_role_unique ON profiles(user_id, role);
